...
This package defines the greenfield v2 contract skeleton for LN Voice Over.
...
Do not use the legacy `source -> split -> parse -> extract -> review ->
synthesis` vocabulary in v2 contracts.
...
- Do not add CLI commands, OCR, parsing algorithms, LLM prompts, TTS rendering,
  visual rendering, migration logic, or plugin frameworks here.
