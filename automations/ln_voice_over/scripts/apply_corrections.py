...
from automations.ln_voice_over.models import Chapter, NarratorStatus
from automations.ln_voice_over.project import load_characters
from automations.ln_voice_over.review import ReviewValidationError, build_reviewed_chapter
...
def _current_narrator_fields(project_dir: Path, chapter: str) -> dict[str, object]:
    manifest_path = project_dir / "chapters" / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    entry = next((e for e in manifest if chapter_id(e) == chapter), None)
    if entry is None:
        print(f"ERROR: Chapter '{chapter}' not in manifest", file=sys.stderr)
        sys.exit(1)
    if "narrator_status" not in entry:
        print(
            "ERROR: Manifest uses the old narrator schema. "
            "Run migrate_narrator_fields before reviewing.",
            file=sys.stderr,
        )
        sys.exit(1)
    return {
        "narrator_status": NarratorStatus(entry["narrator_status"]),
...
def main() -> None:
    ...
    chapter = Chapter.load(parsed_path).model_copy(
        update=_current_narrator_fields(project_dir, padded)
    )
    ...
    try:
        reviewed, changes = build_reviewed_chapter(chapter, original, corrections, registry)
    ...
    report = {
        ...
        "total_segments": len(reviewed.segments),
