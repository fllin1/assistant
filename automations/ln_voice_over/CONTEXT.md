...
- **POV character** vs **Narrator** — resolved: there is only one concept, **Narrator**. The active code uses `narrator_status` and `narrator`; historical `pov_character` artifacts are migrated with `migrate_narrator_fields.py`.
