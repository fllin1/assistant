...
```
AnyFlip book → PDF (via anyflip-downloader) → opendataloader-pdf → raw/volume.txt + illustrations/
```
...
```bash
mv *.pdf ~/.assistant/ln_voice_over/projects/<book-slug>/raw/volume.pdf
```
...
4. Save to `~/.assistant/ln_voice_over/projects/<book-slug>/raw/volume.pdf`
...
This produces:
- `raw/volume.txt` — full text with `Page N` markers between pages
- `illustrations/` — extracted illustration images with a manifest
...
Once you have the PDF in `raw/`, the rest of the pipeline runs as documented in the [README](../README.md):
