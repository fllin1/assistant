...
2. Compile `rules.chapter_headings` patterns with `re.IGNORECASE | re.MULTILINE`. The optional `(?P<num>\d+(?:\.\d+)?)` capture enables numbered chapter and subchapter ids.
3. Automatically detect bare numeric subchapter markers such as `5.1` or OCR-glued markers such as `mp4directs.com6.2` when the marker base matches a preceding numbered chapter heading.
4. Walk `PreparedTextUnit`s in volume order, line by line, and record the first heading match per line.
5. A mid-page heading splits the page text at the heading offset: pre-heading slice stays with the prior chapter, heading + post-heading text starts the new chapter. The same `text_unit_id` therefore contributes to two adjacent segments' `source_unit_ids` when a split occurs — no text is silently relocated.
6. `chapter_id` derivation is position-aware. Numbered headings use `chapter_<num>`; fractional captures use `chapter_<num>_<sub>`; recurrent numbered headings use a `_N` suffix. Non-num headings before the first numbered heading use `chapter_00` and then `chapter_00_N`; non-num headings between numbered headings use `chapter_<previous_num>_<i>`; non-num headings after the last numbered heading use `chapter_<max_num+i>`.
7. `ChapterIndexEntry.order` is 0-indexed dense over the volume order. `ChapterIndexEntry.display_name` is the matched heading line, NFC-normalized and trimmed (`Prologue`, `Chapter 7`, `Chapter 7.1`). Bare numeric subchapter display names are compact (`5.1`, `6.2`). Heading text is also preserved verbatim in the first narration segment of the new chapter so the narrator speaks it at generation.
8. Fallback when no heading matches anywhere: emit a single `chapter_01` with `display_name = "Chapter 1"`; log a `WARNING`.
